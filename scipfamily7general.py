...
def makeUnion(keeperlist):
    # add first and second element wise and divide by 2 if sum of elements is
    union = [element/element if element >=1 else element for element in [sum(x) for x in zip(*keeperlist)]]
    return union
...
def makeweight(c):
    ...
    for i in range(0,len(l)):
        ...
        for j in range(0,len(l[i])):
            if l[i][j]==1:
               plusw=plusw+Fraction(w[j]).limit_denominator()
            else:
               minw=minw+Fraction(w[j]).limit_denominator()
        makeweight.append(plusw-minw)
    return makeweight
        ...
        if f[i]!=0.0:
           ...
           for j in range(len(l)):
               if i!=j:
                  ...
                  k=l.index([max(l[i][0],l[j][0]), max(l[i][1],l[j][1]), max(l[i][2],l[j][2]), max(l[i][3],l[j][3]), max(l[i][4],l[j][4]), max(l[i][5],l[j][5]), max(l[i][6],l[j][6]), ])
                  if k!=j:
...
def allsetsmall(n):
    ...
    return one
def makesn(n,ll):
    biggie = filtersets.powerset(n-2)
    for i in range(len(biggie)):
        for j in range(len(ll)):
            biggie[i].insert(ll[j],0)
           #print biggie[i]
    return biggie
def makesabc(elist,n,ll):
    fin = []
    fini = []
    p = filtersets.powerset(n)
    al= allsets()
    small = [makesn(n,ll)]
    for i in range(len(small)):
        ...
        if filtersets.checkUnionClosed(keep) == 0:
           return 'fucked up'
        keeps = [al.index(x) for x in keep]
        kepa = [al.index(x) for x in keep]
        fina = [0]*len(p)
        final = [0]*len(p)
        for d in range(len(keeps)):
           # print i
            if p[keeps[d]][i]==1:
              # print 'here'
              # print p[keep[d]]
...
def makeobj(c):
    ...
    return makeweight
def scipfamily7general(ll,fixer):
    biggig =[]
    for g in range(len(ll)):
        ...
        for i in range(len(l)):
            new=l[i] + weight
        c = [6,6,6,6,6,5,6]  
        cone = [0,100,41,39,40,27,89]  
        if fixer == 1:
           two = filtersets.zerone(ll[g])
        else:  
           two = ll[g]
        sets=two
        indexkeeps = filtersets.powerset(7)
        allones=[1.0]*len(p)
        flag=1
        count=0
        index=[]
...
def scipfamily7generalmini(lists,fixer,one):
    biggig=[]
    ...
    for g in range(len(lists)):
        ...
        for i in range(len(l)):
            new=l[i] + weight
        c = [6,6,6,6,6,5,7]  
        cone = [0,100,41,39,40,27,7]
        if fixer == 1:
           two1 = filtersets.zerone(lists[g])
        else:  
           two1 = lists[g]
        two =[]
        ...
        while flag==1:
          ...
          if sbin.getStatus()=="unknown" or sbin.getStatus()=="optimal":
             ...
             print c
             print [Fraction(x).limit_denominator() for x in c]
             print [Fraction(x).limit_denominator().numerator for x in c]
             sbin.writeProblem('checkIP.lp')
...
def scipfamily7generalmin(lists):
    biggig=[]
    for g in range(len(lists)):
        two =[]
          ...
          if slin.getStatus()=="optimal":  # if we find optimal print c
             ...
             #print c
              ...
              for i in range(len(c)):
                  c[i]=slin.getVal(WeightVars[i])
              print c
              print 'weeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
              print two
              print g
              break
...
def scipfamily7generalconj(ll,fixer,hole):
    biggig =[]
    for g in range(len(ll)):
        l = [x for x in constraints()]
        p = [x for x in allsets()]
        ...
        for i in range(len(l)):
            new=l[i] + weight
        c = [6,6,6,6,6,5,3,1]  
        cone = [0,100,41,39,40,27,333,432]  
        if fixer == 1:
           two = filtersets.zerone(ll[g])
        else:  
           two = ll[g]
          ...
          if count == 0:
             for i in range(len(index)):
                ...
                # print [indexkeeps.index(p[x]) for x in keeperset]
                 cr=[x for x in makeccoff(index[i])]
                 print cr
                 coeffs = {WeightVars[j]: (2*cr[j]) for j in range(len(cr))}
                 slin.addCons(coeffs, lhs=size, rhs=None,name= 'nogoodcut')
