        ...
        if f[i]!=0.0:
           ...
           for j in range(len(l)):
               if i!=j:
                  ...
                  k=l.index([max(l[i][0],l[j][0]), max(l[i][1],l[j][1]), max(l[i][2],l[j][2]), max(l[i][3],l[j][3]), max(l[i][4],l[j][4]), max(l[i][5],l[j][5]), max(l[i][6],l[j][6]), max(l[i][7],l[j][7]), ])
                  if k!=j:
...
def gcd(a, b):
    """Return greatest common divisor using Euclid's Alorithm."""
    while b:      
        a, b = b, a % b
    return a
...
def makesab(elist,n):
    ...
    return fin
def scipfamily8general(ll,fixer):
    biggig =[]
    for g in range(len(ll)):
        ...
        for i in range(len(l)):
            new=l[i] + weight
        c = [6,6,6,6,6,5,6,7]  
        cone = [0,100,41,39,40,27,89,91]  
        if fixer == 1:
           two = filtersets.zerone(ll[g])
        else:  
           two = ll[g]
        sets=two
        indexkeeps = filtersets.powerset(8)
        allones=[1.0]*len(p)
        flag=1
        count=0
        index=[]
...
def scipfamily8generalmini(ll,fixer,one):
    biggig=[]
    l = [x for x in constraints()]
    p = [x for x in allsets()]
...
def scipfamily8generalconj(ll,fixer):
    biggig =[]
    for g in range(len(ll)):
        l = [x for x in constraints()]
        p = [x for x in allsets()]
