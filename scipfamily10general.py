...
# make the coefficients for WV inequality
def makeweightint(c):
    ...
    return makeweight
# make coefficients for other inequalities in P(\mathcal{A})
def makeccoff(r):
    l=[x for x in allsets()]
    makecoeff=[0]*10
    ...
    return makecoeff
# auxilliary function for debbuging                 
def constraintsf(f):
    l=[x for x in allsets()]
    constraintsf=[]
    ...
    return constraintsf
# generate coefficients for union-closed constraints
def constraints():
    l=[x for x in allsets()]
    constraints=[]
...
# generate a list of uniform n-sets
def makenset(n):
    l=[x for x in allsets()]
    makenset=[]
    for elements in l:
    ...
    return makenset
# generate a list of the indexes of uniform n-sets
def makenseti(n):
    l=[x for x in allsets()]
    makenseti=[]
    for elements in l:
...
# find lmc of two numbers
def lcm(a, b):
    """Return lowest common multiple."""
    return a * b // gcd(a, b)

# find lmc of a bunch of numbers
def lcmm(lst):
    """Return lcm of args."""   
    return reduce(lcm, lst)

# make coefficients for objective function of IP(\mathcal{A},c)
def makeobj(c):
    l=[x for x in allsets()]
...
# An implementation of Algorithm 1, i.e., a cutting plane that determines whether the UC family generated by \mathcal{A} is FC 
# or Non-FC. \mathcal{A} is a list of lists indexing A, and the input lists is [\mathcal{A}]
